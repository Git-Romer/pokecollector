...
for line in lines:
    ...
    if len(parts) >= 5:
        qty = int(parts[1])
        name_raw = parts[2]
        # clean name e.g. "Forest of Vitality (Trainer)" -> "Forest of Vitality"
        name = name_raw.split(' (')[0]
        num_str = parts[4].split(' (')[0]
        
        num = num_str.split('/')[0].lstrip('0')
        ...
        best_match = None
        for c in cards:
            if c.number == num or c.number == num_raw:
                best_match = c
                break
                
        if not best_match:
            print(f"Could not find card: {name} #{num_raw}")
            continue
...
db.commit()
print(f"Successfully added {added} cards.")
