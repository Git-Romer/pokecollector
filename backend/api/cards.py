...
def search_cards(
    ...
    - lang: supported TCGdex language code or "all" for all languages.
      If omitted, the current user's selected catalogue language is used.
    """
    search_lang = _resolve_lang_filter(db, current_user.id, lang)
...
def get_card(
    ...
    requested_lang = normalize_tcgdex_language(lang or get_user_tcgdex_language(db, current_user.id))
