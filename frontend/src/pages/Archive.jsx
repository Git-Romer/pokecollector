...
export default function Archive() {
  ...
  return <section className="archive-card-reveal space-y-10">
    <header><p className="text-sm text-text-muted">John John’s PC</p><h1 className="text-3xl font-semibold text-text-primary">Your Archive</h1><p className="mt-2 text-text-secondary">Everything you’ve chosen to keep, right where it belongs.</p></header>
    ...
    {notes.length > 0 && <section><div className="section-header"><h2 className="section-title">Archive notes</h2><span className="text-sm text-text-muted">John John is keeping watch.</span></div><div className="grid gap-3 lg:grid-cols-3">{notes.map((note) => <ArchiveNote key={note.id} note={note} />)}</div></section>}
  </section>
}
