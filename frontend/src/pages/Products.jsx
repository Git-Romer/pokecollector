...
import PeriodSelector, {getPeriodCutoff, PRODUCT_PERIODS} from '../components/PeriodSelector'
    ...
    return (
        <div className="grid grid-cols-2 gap-3">
            ...
            <div>
                ...
                <select className="select" value={form.product_type}
            ...
            <div>
                ...
                <MoneyInput placeholder={t('products.soldPriceHint')} value={form.sold_price}
                            onChange={(e) => setMoney('sold_price', e.target.value)}/>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" placeholder="e.g. Sealed Box, Shelf"
                       value={form.storage_type} onChange={(e) => set('storage_type', e.target.value)}
                       className="input"/>
            </div>
            <div>
                <label className="text-xs text-text-muted mb-1 block">Storage Detail</label>
                <input type="text" placeholder="e.g. Row A, Shelf 1"
                       value={form.storage_detail} onChange={(e) => set('storage_detail', e.target.value)}
                       className="input"/>
            </div>
            <div className="col-span-2">
        ...
        <div className="bg-bg-elevated/40 border border-border rounded-xl p-4 space-y-4">
            ...
            <div className="grid gap-3 lg:grid-cols-[1.5fr_0.6fr_auto]">
                <div>
                    ...
                    <select className="select" value={collectionItemId}
                            onChange={(e) => setCollectionItemId(e.target.value)}>
                        <option
                            value="">{availableCollectionItems.length ? t('products.chooseCollectionCard') : t('products.noUnlinkedCards')}</option>
                        {availableCollectionItems.map(({item, available}) => (
                            <option key={item.id}
                                    value={item.id}>{collectionItemLabel(item, formatPrice)} · {t('products.available')}: {available}</option>
                        ))}
                    </select>
                </div>
                <div>
                    <label className="text-xs text-text-muted mb-1 block">{t('common.quantity')}</label>
                    <input type="number" min="1" max={selectedAvailable || 999} step="1" className="input"
                           value={linkQuantity} onChange={(e) => setLinkQuantity(e.target.value)}/>
                </div>
                <div className="flex items-end">
                    <button disabled={!canLink || loading} onClick={submitLink}
                        ...
                        return (
                            <div key={entry.id} className="bg-bg-card border border-border rounded-lg p-3 space-y-3">
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <button disabled={loading || entry.sold_quantity > 0}
                                            onClick={() => onUnlink(product.id, entry.id)}
                                ...
                                {entry.active_quantity > 0 && (
                                    <div className="grid gap-2 md:grid-cols-[0.5fr_0.8fr_0.8fr_1fr_auto]">
                                        <input type="number" min="1" max={entry.active_quantity} step="1"
                                               className="input text-sm py-1.5" value={form.quantity}
                                               onChange={(e) => updateSaleForm(entry.id, 'quantity', e.target.value)}
                                               aria-label={t('common.quantity')}/>
                                        <MoneyInput className="input text-sm py-1.5"
                                                    placeholder={t('products.saleTotal')} value={form.sold_price}
                                                    onChange={(e) => updateSaleForm(entry.id, 'sold_price', e.target.value)}/>
                                        <input type="date" className="input text-sm py-1.5" value={form.sold_date}
                                               onChange={(e) => updateSaleForm(entry.id, 'sold_date', e.target.value)}/>
                                        <input type="text" className="input text-sm py-1.5"
                                               placeholder={t('products.saleNotes')} value={form.notes}
                                               onChange={(e) => updateSaleForm(entry.id, 'notes', e.target.value)}/>
                                        <button disabled={loading || !canSell} onClick={() => submitSale(entry)}
                                        ...
                                        {(entry.ledger_entries || []).map(ledger => (
                                            <p key={ledger.id}
                                               className="text-xs text-text-muted flex items-center gap-1">
                                                <History
                                                    size={12}/> {ledger.event_date}: {ledger.quantity} × {entry.card?.name || entry.card_id} · {formatPrice(ledger.amount)}{ledger.notes ? ` · ${ledger.notes}` : ''}
                                            </p>
                                        ))}
                                    </div>
                                )}
            ...
            <div className="grid gap-2 md:grid-cols-[0.8fr_0.8fr_1fr_auto] pt-3 border-t border-border">
                <MoneyInput className="input text-sm py-1.5" placeholder={t('products.flatGainAmount')}
                            value={flatGain.amount}
                            onChange={(e) => setFlatGain(prev => ({...prev, amount: e.target.value}))}/>
                <input type="date" className="input text-sm py-1.5" value={flatGain.event_date}
                       onChange={(e) => setFlatGain(prev => ({...prev, event_date: e.target.value}))}/>
                <input type="text" className="input text-sm py-1.5" placeholder={t('products.flatGainNotes')}
                       value={flatGain.notes}
                       onChange={(e) => setFlatGain(prev => ({...prev, notes: e.target.value}))}/>
                <button disabled={loading || !canAddFlatGain} onClick={submitFlatGain}
                ...
                <div className="space-y-1">
                    {(product.ledger_entries || []).map(entry => (
                        <p key={entry.id} className="text-xs text-text-muted flex items-center gap-1">
                            <History
                                size={12}/> {entry.event_date}: {t('products.flatGain')} · {formatPrice(entry.amount)}{entry.notes ? ` · ${entry.notes}` : ''}
                        </p>
                    ))}
                </div>
            )}
        ...
        <div className="space-y-4 pb-2">
            ...
            <div className="flex items-center justify-between gap-2 mb-4 flex-wrap">
                <div className="min-w-0">
                    <h1 className="text-5xl font-bold text-text-primary mag-heading uppercase leading-none mt-2">
                        <SplitText text={t('products.title')} delay={40}/></h1>
                ...
                <div className="card">
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={monthlyChartData}>
                            ...
                            <Bar dataKey="current" name={t('products.currentValue')} fill="#EF1515"
                                 radius={[4, 4, 0, 0]}/>
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            ...
            {creating && (
                <div className="card border-brand-red/30">
                    ...
                    <ProductForm onSubmit={(data) => createMutation.mutate(data)} onCancel={() => setCreating(false)}
                ...
                <div className="card space-y-3">
                    <div className="flex flex-wrap items-center gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <select className="select text-sm py-1.5 w-40" value={sortBy}
                    ...
                    {showFilters && (
                        <div className="pt-3 border-t border-border grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div>
                                ...
                                <select className="select text-sm py-1.5" value={filterType}
                            ...
                            <div>
                                <label
                                    className="text-xs text-text-muted mb-1 block">{t('products.filterDateFrom')}</label>
                            ...
                            <div>
                                <label
                                    className="text-xs text-text-muted mb-1 block">{t('products.filterDateTo')}</label>
                            ...
                            <div className="flex items-end">
                                <span
                                    className="text-xs text-text-muted">{filteredAndSorted.length} / {products.length} {t('products.items')}</span>
                            </div>
                        </div>
                    )}
                </div>
                                    ...
                                    <tr className="border-b border-border/50 hover:bg-bg-elevated/50">
                                        {editingId === p.id ? (
                                            <td colSpan={8} className="px-4 py-4">
                                                <ProductForm initial={p} onSubmit={(data) => updateMutation.mutate({
                                                    id: p.id,
                                                    data
                                                })}
                                            ...
                                            <>
                                                <td className="px-4 py-3">
                                                    ...
                                                    {p.notes &&
                                                        <p className="text-xs text-text-muted truncate max-w-[160px]">{p.notes}</p>}
                                                    {p.sold_date && <span
                                                        className="badge badge-green text-xs">{t('common.sold')}</span>}
                                                </td>
                                        ...
                                        <tr className="border-b border-border/50">
                                            <td colSpan={8} className="px-4 py-4">
                                                <ProductLedgerPanel
                                                    ...
                                                    onLink={(productId, data) => linkCardMutation.mutateAsync({
                                                        productId,
                                                        data
                                                    })}
                                                    onUnlink={(productId, productCardId) => unlinkCardMutation.mutateAsync({
                                                        productId,
                                                        productCardId
                                                    })}
                                                    onSell={(productId, productCardId, data) => sellCardMutation.mutateAsync({
                                                        productId,
                                                        productCardId,
                                                        data
                                                    })}
                                                    onFlatGain={(productId, data) => flatGainMutation.mutateAsync({
                                                        productId,
                                                        data
                                                    })}
                                                />
                    ...
                    <div className="md:hidden space-y-2 p-2">
                        {filteredAndSorted.map((p) => {
                            if (editingId === p.id) {
                                return (
                                    <div key={p.id}
                                         className="bg-bg-card border border-border rounded-lg p-3 space-y-3">
