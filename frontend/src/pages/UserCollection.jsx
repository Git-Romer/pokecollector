...
export default function UserCollection() {
    ...
    const rarities = useMemo(() => {
        const all = new Set()
        items.forEach(item => {
            if (item.card?.rarity) all.add(item.card.rarity)
        })
        return [...all].sort()
    }, [items])
    ...
    const variants = useMemo(() => {
        const all = new Set()
        items.forEach(item => {
            if (item.variant) all.add(item.variant)
        })
        return [...all].sort()
    }, [items])
        ...
        result.sort((a, b) => {
            ...
            switch (sortBy) {
                case 'name':
                    ...
                    break
                case 'price':
                    valA = getEffectiveCardPrice(a.card, a.variant, pricePrimaryField);
                    valB = getEffectiveCardPrice(b.card, b.variant, pricePrimaryField);
                    break
                case 'quantity':
                    valA = a.quantity;
                    valB = b.quantity;
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                        <h1 className="text-5xl font-bold text-text-primary mag-heading uppercase leading-none mt-2">
                            <SplitText text={t('collection.userCollection')} delay={40}/></h1>
                    ...
                    <div className="mt-3 pt-3 border-t border-border grid grid-cols-2 sm:grid-cols-4 gap-3">
                        ...
                        <div>
                            ...
                            <div className="flex gap-1">
                                <select className="select py-1.5 text-sm flex-1" value={sortBy}
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                    {filtered.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <div
                                    className="aspect-[2.5/3.5] rounded-xl overflow-hidden ring-1 ring-white/5 group-hover:ring-brand-red/30 transition-all">
                                ...
                                <div className="mt-1 px-0.5">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span
                                            className="text-[9px] text-text-muted">{item.quantity}x · {item.variant || 'Normal'}</span>
                                        {price > 0 && (
                                            <span
                                                className="text-[9px] font-bold text-green">{formatPrice(price)}</span>
                                        )}
                                    </div>
                                </div>
                            </div>
