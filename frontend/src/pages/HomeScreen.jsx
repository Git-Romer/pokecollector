...
import {getDashboard, getInvestmentTracker, getSyncStatus, triggerPriceSync} from '../api/client'
...
function CardThumb({card, onClick}) {
    ...
    return (
        <div ref={ref} className="flex-shrink-0 w-[110px] cursor-pointer group" onClick={onClick}
...
export default function HomeScreen() {
    ...
    const {data: investmentData = []} = useQuery({
        ...
        queryFn: () => getInvestmentTracker({
            period: activePeriod?.apiPeriod ?? 'max',
    ...
    return (
        <div className="min-h-dvh flex flex-col relative overflow-x-hidden">
            ...
            <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
                <div style={{
                    position: 'absolute', top: '3%', left: '8%', width: 300, height: 300, borderRadius: '50%',
                    background: 'radial-gradient(circle, rgba(227,0,11,0.07) 0%, transparent 70%)',
                    animation: 'float-orb 9s ease-in-out infinite'
                }}/>
                <div style={{
                    position: 'absolute', bottom: '12%', right: '4%', width: 240, height: 240, borderRadius: '50%',
            ...
            <div className="relative z-10 flex flex-col gap-6 px-4 pt-6 pb-10">
                ...
                <div className="text-center -mt-2">
                    ...
                    <div className="mb-1 flex items-center justify-center gap-2 truncate max-w-[90vw] mx-auto">
                        ...
                        <p className="text-sm font-semibold truncate" style={{color: 'rgba(255,255,255,0.55)'}}>
                            {t('home.hello')}, <span className="font-black"
                                                     style={{color: '#f5c842'}}>{trainerName}</span>! 👋
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {STAT_CARDS.map(stat => (
                        ...
                        >
                            ...
                            <p className="text-sm sm:text-base font-black leading-none truncate max-w-full"
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <button onClick={() => navigate('/collection')}
                                    ...
                                    style={{color: '#e3000b'}}>{t('home.viewAll')} →
                            </button>
                        </div>
                ...
                {topCards.length > 0 && (
                    <div>
                        ...
                        <div className="flex gap-2.5 overflow-x-auto pb-1 no-scrollbar -mx-4 px-4">
                            {topCards.map((card, i) => (
                                <div key={card.collection_item_id || card.id}
                      ...
                      group-hover:scale-[1.03]"
                                            ...
                                            <CardImage src={resolveCardImageUrl(card)} alt={card.name}
                                                       className="w-full h-full object-cover"/>
                                        </div>
                                        <span
                                            className="absolute top-1 left-1 text-[9px] font-black px-1 rounded leading-4"
