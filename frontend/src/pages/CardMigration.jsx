...
import {ArrowRight, CheckCircle, Info, XCircle, Zap} from 'lucide-react'
import {dismissCustomMatch, getCustomMatches, migrateCustomCard} from '../api/client'
    ...
    return (
        <div className="flex flex-col items-center gap-2">
            ...
            <div className="text-center max-w-[120px]">
                ...
                {card.is_custom && (
                    <span
                        className="inline-block mt-1 text-xs bg-yellow/20 text-yellow px-1.5 py-0.5 rounded-full">{t('migration.custom')}</span>
                )}
            </div>
    ...
    return (
        <div className="card space-y-4">
            ...
            <div className="flex gap-3">
                <button onClick={() => onMigrate(match.match_id)} disabled={migrating || dismissing}
                        className="btn-primary flex-1 justify-center">
                    <CheckCircle size={15}/> {migrating ? t('migration.migrating') : t('migration.migrate')}
                </button>
                <button onClick={() => onDismiss(match.match_id)} disabled={migrating || dismissing}
...
export default function CardMigration() {
    ...
    const migrateMutation = useMutation({
        ...
        onError: () => {
            toast.error(t('migration.migrateError'));
            setActionId(null);
            setActionType(null)
        },
    })
    ...
    const dismissMutation = useMutation({
        ...
        onError: () => {
            toast.error(t('migration.dismissError'));
            setActionId(null);
            setActionType(null)
        },
    })

    const handleMigrate = (matchId) => {
        setActionId(matchId);
        setActionType('migrate');
        migrateMutation.mutate(matchId)
    }
        ...
        <div className="space-y-4 pb-2 max-w-2xl">
            <div>
                <h1 className="text-5xl font-bold text-text-primary mag-heading uppercase leading-none mt-2"><SplitText
                    text={t('migration.title')} delay={40}/></h1>
