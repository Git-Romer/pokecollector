...
import {ArrowUpDown, Award, Trophy} from 'lucide-react'
...
function TrainerAvatar({avatarId, username}) {
    if (!avatarId) {
        return <img src="/pokeball.svg" alt={username}
                    className="h-12 w-12 rounded-full border border-border bg-bg-card p-2"/>
    }
    ...
    return (
        <div className="page-container">
            ...
            <div className="card relative overflow-hidden">
                <div
                    className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_top_right,rgba(255,213,79,0.16),transparent_40%)]"/>
                        ...
                        <div className="relative">
                            ...
                            <select className="select pl-9" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                                {SORT_OPTIONS.map((key) => (
                                    <option key={key}
                                            value={key}>{t(`leaderboard.${key === 'total_value' ? 'value' : key === 'total_cards' ? 'cards' : key === 'unique_cards' ? 'uniqueCards' : key === 'sets_completed' ? 'setsCompleted' : 'pnl'}`)}</option>
                                ))}
                            </select>
                        </div>
                    </div>
                            ...
                            >
                                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                    <div className="flex items-start gap-3">
                                        <div
                                            className="flex h-12 min-w-12 items-center justify-center rounded-2xl bg-bg-primary text-lg font-black">
                                            {rankLabel(index + 1)}
                                        </div>
                                            ...
                                            {trainer.user_id !== currentUser?.id && (
                                                <div className="flex gap-2 mt-1">
                                                    <button
                                                        ...
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            navigate(`/collection/user/${trainer.user_id}`)
                                                        }}
                                        ...
                                        <div className="col-span-2 md:col-span-1">
                                            ...
                                            {bestCard ? (
                                                <div className="mt-1 flex items-center gap-2">
                                                    <div
                                                        className="h-10 w-8 overflow-hidden rounded-lg border border-border bg-bg-primary">
