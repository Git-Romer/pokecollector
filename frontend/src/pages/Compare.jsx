...
function TrainerAvatar({avatarId, username}) {
    if (!avatarId) {
        return <img src="/pokeball.svg" alt={username}
                    className="h-16 w-16 rounded-full border border-border bg-bg-card p-3"/>
    }
    ...
    return (
        <div className="card space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
                <StatBlock label={t('leaderboard.value')} value={formatPrice(trainer?.total_value ?? 0)}
                           accent="text-yellow"/>
                <StatBlock label={t('leaderboard.cards')} value={Number(trainer?.total_cards ?? 0).toLocaleString()}/>
                <StatBlock label={t('leaderboard.uniqueCards')}
                           value={Number(trainer?.unique_cards ?? 0).toLocaleString()}/>
                <StatBlock label={t('leaderboard.setsCompleted')}
                           value={Number(trainer?.sets_completed ?? 0).toLocaleString()}/>
            </div>
                ...
                {bestCard ? (
                    <div className="flex items-center gap-3 rounded-2xl border border-border bg-bg-primary/60 p-3">
                        <div className="h-16 w-12 overflow-hidden rounded-lg border border-border bg-bg-card">
                            <img src={resolveCardImageUrl(bestCard)} alt={bestCard.name}
                    ...
                    <div
                        className="rounded-2xl border border-border bg-bg-primary/60 p-3 text-sm text-text-muted">-</div>
                )}
            </div>
        </div>
    )
                    ...
                    <div className="grid gap-4 lg:grid-cols-[1fr_320px_1fr]">
                        ...
                        <div className="card flex flex-col items-center justify-center gap-4 text-center">
                            <div className="relative flex h-44 w-full max-w-[260px] items-center justify-center">
                                <div
                                    className="absolute left-4 top-1/2 flex h-28 w-28 -translate-y-1/2 items-center justify-center rounded-full border border-blue/30 bg-blue/10 text-3xl font-black text-blue">
                                    {data.only_a}
                                </div>
                                <div
                                    className="absolute right-4 top-1/2 flex h-28 w-28 -translate-y-1/2 items-center justify-center rounded-full border border-brand-red/30 bg-brand-red/10 text-3xl font-black text-brand-red">
                                    {data.only_b}
                                </div>
                                <div
                                    className="relative z-10 flex h-24 w-24 items-center justify-center rounded-full border border-yellow/30 bg-yellow/10 text-3xl font-black text-yellow">
                                    {data.overlap}
                                </div>
                            </div>

                            <div className="w-full space-y-3">
                                <StatBlock label={t('compare.cardsInCommon')}
                                           value={Number(data.overlap ?? 0).toLocaleString()} accent="text-yellow"/>
                                <StatBlock label={t('compare.onlyA')} value={Number(data.only_a ?? 0).toLocaleString()}
                                           accent="text-blue"/>
                                <StatBlock label={t('compare.onlyB')} value={Number(data.only_b ?? 0).toLocaleString()}
                                ...
                                <div>
                                    ...
                                    <div className="h-2 overflow-hidden rounded-full bg-bg-primary">
                                        <div className="h-full rounded-full bg-yellow transition-all"
                                             style={{width: `${overlapRatio}%`}}/>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {data.trade_suggestions?.length ? (
                            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                                {data.trade_suggestions.map((trade) => (
                                    <div key={`${trade.card_id}-${trade.owner_username}-${trade.wants_username}`}
                                         className="rounded-2xl border border-border bg-bg-primary/60 p-3">
                                        <div className="flex items-center gap-3">
                                            <div
                                                className="h-16 w-12 overflow-hidden rounded-lg border border-border bg-bg-card">
                                                <img src={resolveCardImageUrl(trade)} alt={trade.card_name}
                                                     className="h-full w-full object-cover" loading="lazy"/>
                                            </div>
                                            <div className="min-w-0">
                            ...
                            <div
                                className="rounded-2xl border border-dashed border-border bg-bg-primary/50 p-6 text-center text-text-muted">
                                {t('compare.noTrades')}
                            </div>
                        )}
                    </div>
