...
import {ArrowLeft, Check, Heart, Plus, Trash2, X} from 'lucide-react'
import {
    addToCollection,
    addToWishlist,
    getSetChecklist,
            ...
            >
                ...
                <div className="p-5">
                    ...
                    <div className="space-y-4">
                        <form onSubmit={submitAddVersion}
                              className="space-y-3 rounded-xl border border-brand-red/30 bg-bg-card p-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <select value={addCondition} onChange={(e) => setAddCondition(e.target.value)}
                                            className="select">
                                        {CONDITIONS.map(condition => <option key={condition}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                <button type="submit" disabled={isAdding || !exchangeRateReady}
                                        className="btn-primary justify-center">
                                    <Plus
                                        size={14}/> {isAdding ? t('card.adding') : t('collection.addVersionToCollection')}
                                </button>
                                <button
                                    type="button"
            ...
            <div className="card">
                <div className="flex items-start gap-4">
                    {resolveSetImageUrl(set, 'logo') && (
                        <img src={resolveSetImageUrl(set, 'logo')} alt={set.name}
                             className="h-12 sm:h-16 object-contain flex-shrink-0 max-w-[120px]"/>
                    )}
                    <div className="flex-1 min-w-0">
                    ...
                    <div key={card.id}
                        ...
                        {resolveCardImageUrl(card) ? (
                            <img src={resolveCardImageUrl(card)} alt={card.name}
                                 className="w-full aspect-[2.5/3.5] object-cover" loading="lazy"/>
                        ) : (
                            <div
                        ...
                        {card.quantity > 1 && (
                            <div
                                className="absolute top-0.5 left-0.5 bg-bg-surface/90 rounded text-xs px-1 text-text-primary font-bold">
                                {card.quantity}x
                            </div>
                        )}
