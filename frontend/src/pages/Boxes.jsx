...
export default function Boxes() {
  ...
  return <section className="space-y-6 archive-card-reveal"><header className="flex items-end justify-between gap-4"><div><p className="text-sm text-text-muted">Storage room</p><h1 className="text-3xl font-semibold">Boxes</h1><p className="mt-2 text-text-secondary">A calm place for the parts of your collection that belong together.</p></div><Button appearance="primary" icon={<AddRegular />} onClick={() => setOpen(true)}>New Box</Button></header>{isLoading ? <Spinner label="Opening boxes" /> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{boxes.map((box) => <Link key={box.id} to={`/boxes/${box.id}`}><Card className="archive-box"><BoxRegular fontSize={32} /><Text weight="semibold" size={500}>{box.name}</Text><Text size={200}>{box.description || 'Ready for cards.'}</Text></Card></Link>)}</div>}<Dialog open={open} onOpenChange={(_, data) => setOpen(data.open)}><DialogSurface><DialogBody><DialogContent><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); if (name.trim()) create.mutate(name.trim()) }}><Field label="Box name"><Input autoFocus value={name} onChange={(_, data) => setName(data.value)} /></Field><Button type="submit" appearance="primary" disabled={!name.trim() || create.isPending}>Create Box</Button></form></DialogContent></DialogBody></DialogSurface></Dialog></section>
}
