...
function TrainerAvatar({avatarId, username}) {
    if (!avatarId) {
        return <img src="/pokeball.svg" alt={username}
                    className="h-14 w-14 rounded-full border border-border bg-bg-card p-3"/>
    }
    ...
    return (
        <div className="page-container">
            ...
            <div className="card relative overflow-hidden">
                <div
                    className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_top_right,rgba(255,138,101,0.16),transparent_40%)]"/>
                <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    {isOtherUser && (
                        <button onClick={() => navigate(-1)}
                                className="flex items-center gap-1.5 text-sm text-text-muted hover:text-text-primary transition-colors mb-2 sm:mb-0 sm:absolute sm:-top-1 sm:left-0">
                            <ArrowLeft size={14}/> {t('common.back')}
                        </button>
                    )}
                    <div className="flex items-center gap-3">
                        <TrainerAvatar avatarId={data?.avatar_id || user?.avatar_id}
                                       username={data?.username || user?.username || 'Trainer'}/>
                        <div>
                    ...
                    <div
                        className="rounded-2xl border border-border bg-bg-primary/70 px-4 py-3 text-sm font-semibold text-yellow">
                        {earnedLabel}
                    </div>
                </div>
            </div>
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-start justify-between gap-2">
                                            ...
                                            <span
                                                className={`badge ${achievement.unlocked ? 'badge-yellow' : 'badge-gray'}`}>
