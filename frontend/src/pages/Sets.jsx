        ...
        <div className="space-y-4 pb-2">
            ...
            <div className="flex items-center justify-between gap-2 mb-4 flex-wrap">
                ...
                <div className="flex gap-2">
                    {newSets.length > 0 && (
                        <button onClick={() => markSeenMutation.mutate(newSets.map(set => set.id))}
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        {hiddenSetCount > 0 && (
                            ...
                            >
                                ...
                                <span
                                    className="rounded-full bg-bg-elevated px-1.5 py-0.5 text-[10px] text-text-secondary">{hiddenSetCount}</span>
                            </button>
                        )}
                        {hasActiveFilters && (
                        ...
                        <div className="relative z-10 flex items-center justify-between p-6 gap-4">
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-3 mb-2">
                                    <span
                                        className="text-sm text-text-secondary">{heroOwned}/{heroTotal} {t('sets.heroCards')}</span>
                                    <span
                            ...
                            <div className="flex-shrink-0 w-36 h-28 flex items-center justify-center">
                                {resolveSetImageUrl(hero, 'logo')
                                    ? <img src={resolveSetImageUrl(hero, 'logo')} alt={hero.name}
                                           className="set-hero-logo group-hover:scale-105 transition-transform duration-300"/>
                    ...
                    {filtered.map((set) => {
                        ...
                        return (
                            ...
                            >
                                {set.is_new && (
                                    <span
                                        className="absolute top-2 left-2 z-10 badge badge-red">{t('common.new')}</span>
                                )}
                                <button
                                    type="button"
                                    onClick={(event) => {
                                ...
                                {isHidden && (
                                    <span
                                        className="absolute top-2 right-2 z-10 rounded-full border border-border bg-bg-card/90 px-2 py-0.5 text-[10px] font-bold text-text-secondary">
                    {t('sets.hidden')}
                  </span>
                                )}
                                ...
                                <div
                                    className="h-28 sm:h-32 bg-bg-elevated flex items-center justify-center relative overflow-hidden">
                                    ...
                                    {resolveSetImageUrl(set, 'logo') ? (
                                        ...
                                        <img src="/pokemon-logo.svg"
                                             className="w-28 h-12 object-contain opacity-40 relative z-10" alt=""/>
                                    )}

                                    {pct === 100 && !isHidden && (
                                        <span
                                            className="absolute top-2 right-2 z-10 bg-green/20 text-green text-[10px] font-black px-2 py-0.5 rounded-full border border-green/30">
                      ★ {t('sets.filterComplete')}
                                ...
                                <div className="p-3">
                                    <div className="flex items-start gap-1.5 mb-0.5">
                                        ...
                                        {set.lang && (
                                            <span
                                                className={`flex-shrink-0 text-[9px] font-black px-1.5 py-0.5 rounded leading-none ${tcgdexLanguageBadgeClass(set.lang)}`}>
                        {tcgdexLanguageLabel(set.lang)}
                                    ...
                                    <p className="text-[11px] text-text-muted mb-0.5">
                                        {set.abbreviation && (
                                            <span
                                                className="font-mono font-bold text-text-secondary mr-1">{set.abbreviation}</span>
                                        )}
                                        {set.series}
                                    ...
                                    <div className="flex items-center justify-between text-[10px] mb-1.5">
                                        ...
                                        <span
                                            className={`font-bold ${pct === 100 ? 'text-green' : 'text-text-secondary'}`}>
                      {pct}%
                    </span>
                                    </div>
