...
import {
    ...
    applyBinderPrintOptimization,
    exportBinderCsv,
    getApiErrorMessage,
    getBinderCards,
    getBinderEntryEquivalentPrints,
    getBinderPrintOptimization,
    getCollection,
    importBinderCsv,
    removeBinderEntry,
    removeCardFromBinder,
    searchCards,
    switchBinderEntryCard,
    updateBinderEntry
} from '../api/client'
            ...
            >
                ...
                <div className="p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <button type="button" onClick={onChooseFile} disabled={isImporting}
                                className="btn-primary justify-center">
                            <Upload
                                size={16}/> {isImporting ? t('binderTypes.importingCsv') : t('binderTypes.importCsv')}
                        </button>
                    ...
                    <div className="rounded-xl bg-bg-elevated/35 p-3 text-xs text-text-secondary space-y-3">
                        <div className="space-y-1">
                            ...
                            <p className="font-mono text-[11px] text-text-primary">
                                <span className="text-brand-red">BLK</span> → set_code · <span
                                className="text-brand-red">057</span> → number
                            </p>
                        </div>
                        ...
                        <div className="border-t border-white/5 pt-3 space-y-2">
                            ...
                            <code
                                className="block overflow-x-auto rounded-lg bg-bg/70 px-3 py-2 text-[11px] text-text-primary font-mono">
                        ...
                        <div className="border-t border-white/5 pt-3 space-y-2">
                            ...
                            <div className="rounded-lg bg-bg/60 px-3 py-2">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-[11px]">
                                    <div><span
                                        className="font-mono text-text-primary">required_quantity</span><br/><span
                                        className="text-text-muted">1</span></div>
                                    <div><span className="font-mono text-text-primary">lang</span><br/><span
                                        className="text-text-muted">en</span></div>
                                </div>
                            </div>
...
export default function BinderDetail() {
    ...
    const updateEntryMutation = useMutation({
        mutationFn: ({
                         binderCardId,
                         requiredQuantity
                     }) => updateBinderEntry(parseInt(binderId), binderCardId, {required_quantity: requiredQuantity}),
    ...
    const wishlistMutation = useMutation({
        mutationFn: ({
                         binderCardId,
                         quantity = null
                     }) => addBinderEntryToWishlist(parseInt(binderId), binderCardId, quantity),
        onSuccess: (result) => {
            if (result?.added > 0) {
    ...
    const switchPrintMutation = useMutation({
        mutationFn: ({
                         ...
                         cardId,
                         collectionItemId
                     }) => switchBinderEntryCard(parseInt(binderId), binderCardId, cardId, collectionItemId),
        onSuccess: () => {
    ...
    const {
        data: printOptimizationData,
        isLoading: printOptimizationLoading,
        isError: printOptimizationError,
        error: printOptimizationErrorData
    } = useQuery({
    ...
    return (
        <div className="space-y-4 pb-2">
                ...
                <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {binder?.icon_pokemon_id ? (
                            <img src={`${SPRITE_BASE_URL}/${binder.icon_pokemon_id}.gif`} alt=""
                                 className="h-8 w-8 pixelated flex-shrink-0" loading="lazy"/>
                        ) : isWishlist ? (
            ...
            <div
                className={isWishlist ? "grid grid-cols-2 md:grid-cols-5 gap-2" : "grid grid-cols-2 md:grid-cols-4 gap-2"}>
                <div className="card p-3"><p className="text-xs text-text-muted">{t('binderTypes.required')}</p><p
                    className="text-lg font-bold text-text-primary">{totalCount}</p></div>
                <div className="card p-3"><p className="text-xs text-text-muted">{t('binderTypes.owned')}</p><p
                    className="text-lg font-bold text-green">{ownedCount}</p></div>
                <div className="card p-3"><p className="text-xs text-text-muted">{t('binderTypes.missing')}</p><p
                    className="text-lg font-bold text-brand-red">{missingCount}</p></div>
                {isWishlist && (
            ...
            {(binder?.format || isWishlist) && (
                <div className="card p-3 flex items-center justify-between gap-3 flex-wrap">
                    ...
                    {binder?.format && <span
                        className="text-xs px-2 py-1 rounded-full bg-yellow/15 text-yellow font-semibold">{binder.format}</span>}
                </div>
            )}
                ...
                <div className="card border-brand-red/20">
                    ...
                    {isWishlist && (
                        <>
                            ...
                            {searchResults?.data && (
                                <div
                                    className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2 max-h-64 overflow-y-auto">
                                        ...
                                        return (
                                            <div key={card.id}
                                                 ...
                                                 onClick={() => {
                                                     ...
                                                     if (requiredQuantity) addMutation.mutate({
                                                         cardId: card.id,
                                                         requiredQuantity
                                                     })
                                                 }}>
                                                {(card.images?.small || resolveCardImageUrl(card) || card.image) ? (
                                                    ...
                                                    <div
                                                        className="w-full aspect-[2.5/3.5] bg-bg-card flex items-center justify-center text-xs text-text-muted p-1 text-center">
                                                        {card.name}
                                                    </div>
                                                )}
                                                {!alreadyAdded && (
                                    ...
                                    {collectionSearchResults.map((item) => {
                                        ...
                                        return (
                                            <div key={`${card.id}-${item.id}`}
                                                ...
                                                <div
                                                    className="absolute top-0.5 left-0.5 bg-bg/80 text-text-primary text-xs rounded px-1">{item.quantity}x
                                                </div>
                                                {(item.variant || item.condition) && (
                                                    <div
                                                        className="absolute bottom-0 left-0 right-0 bg-black/70 text-white text-[9px] text-center truncate px-1">
                                                        {[item.variant || 'Normal', item.condition].filter(Boolean).join(' · ')}
                                                    </div>
                                                )}
                                                {unavailable && !alreadyAdded && (
                                                    <div
                                                        className="absolute inset-0 bg-black/65 flex items-center justify-center text-white text-[10px] text-center px-1">
            ...
            {cards.length > 0 && (
                <div className="card p-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
                    ...
                    />
                    <select className="select text-sm py-2" value={binderFilterSet}
                            onChange={(e) => setBinderFilterSet(e.target.value)}>
                        <option value="">{t('binderTypes.allSets')}</option>
                        {binderSets.map(setName => <option key={setName} value={setName}>{setName}</option>)}
                    </select>
                    <select className="select text-sm py-2" value={binderFilterStatus}
                            onChange={(e) => setBinderFilterStatus(e.target.value)}>
            ...
            {cards.length === 0 ? (
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2 sm:gap-3">
                    {visibleCards.map((card) => {
                        ...
                        return (
                            <TiltBinderCard key={card.binder_card_id || card.id}
                                            className="relative group rounded-xl overflow-hidden card p-0 cursor-pointer"
                                ...
                                {resolveCardImageUrl(card) ? (
                                    ...
                                    <div
                                        className={`w-full aspect-[2.5/3.5] bg-bg-card flex items-center justify-center text-xs text-text-muted p-1 text-center ${isMissing ? 'grayscale opacity-60' : ''}`}>
                                        {card.name}
                                    </div>
                                )}
                                <div className="p-1.5">
                                ...
                                {isWishlist && (
                                    <div
                                        className={`absolute top-1 left-1 rounded-full text-white text-xs px-1.5 py-0.5 font-medium ${
                                ...
                                {!isWishlist && card.in_collection && (
                                    <div
                                        className="absolute top-1 left-1 bg-green/80 rounded-full text-white text-xs px-1">
                                        {card.quantity}x
                                    </div>
                                )}
                            </TiltBinderCard>
            ...
            {showPrintOptimizer && (
                <div
                    className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm md:flex md:items-center md:justify-center md:bg-black/80"
                    onClick={() => setShowPrintOptimizer(false)}>
                    <div
                    ...
                    >
                        ...
                        <div className="p-5 space-y-4">
                            <div className="flex items-start justify-between gap-3">
                                ...
                                <button onClick={() => setShowPrintOptimizer(false)}
                            ...
                            {printOptimizationLoading &&
                                <p className="text-sm text-text-muted text-center py-6">{t('binderTypes.optimizingPrints')}</p>}
                                ...
                                <>
                                    ...
                                    <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-1">
                                        {printOptimizationData.recommendations.map((item) => {
                                            ...
                                            return (
                                                <div key={item.binder_card_id}
                                                     className={`rounded-xl border p-3 space-y-2 ${isSelected ? 'border-yellow/40 bg-yellow/5' : 'border-border bg-bg-card/60'}`}>
                                                        ...
                                                        <div className="min-w-0 flex-1 space-y-2">
                                                            <div
                                                                ...
                                                                <div className="min-w-0 flex items-center gap-2">
                                                                    {resolveCardImageUrl(item.current) &&
                                                                        <img src={resolveCardImageUrl(item.current)}
                                                                             alt={item.current.name}
                                                                             className="w-9 aspect-[2.5/3.5] object-cover rounded"
                                                                             loading="lazy"/>}
                                                                    <div className="min-w-0">
                                                                        ...
                                                                        {(item.current.variant || item.current.condition) &&
                                                                            <p className="text-[10px] text-text-muted truncate">{[item.current.variant, item.current.condition].filter(Boolean).join(' · ')}</p>}
                                                                    </div>
                                                                </div>
                                                                <span className="text-text-muted text-xs">→</span>
                                                                <div
                                                                    className="min-w-0 flex items-center gap-2 justify-end text-right">
                                                                    <div className="min-w-0">
                                                                        ...
                                                                        {(item.suggested.variant || item.suggested.condition) &&
                                                                            <p className="text-[10px] text-text-muted truncate">{[item.suggested.variant, item.suggested.condition].filter(Boolean).join(' · ')}</p>}
                                                                    </div>
                                                                    {resolveCardImageUrl(item.suggested) &&
                                                                        <img src={resolveCardImageUrl(item.suggested)}
                                                            ...
                                                            <p className="text-[11px] text-text-muted">
                                                                ...
                                                                · {t('binderTypes.estimatedSavings')}: {formatPrice(item.total_savings)}
                                                            </p>
                                                        </div>
                                                    </label>
                                                </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button type="button" className="btn-ghost justify-center"
                                                onClick={() => setShowPrintOptimizer(false)}>{t('common.cancel')}</button>
                                        <button
                                            type="button"
                                            className="btn-primary justify-center"
                ...
                <div
                    ...
                    >
                        <div className="p-4 space-y-4">
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                    <h2 id="binder-card-dialog-title"
                                        className="text-lg font-bold text-text-primary truncate">{selectedCard.name}</h2>
                                    <p className="text-xs text-text-muted">{selectedCard.set_name || selectedCard.set_id} #{selectedCard.number}</p>
                                </div>
                                <button ref={selectedCardCloseRef} onClick={() => setSelectedCard(null)}
                                        className="text-text-muted hover:text-text-primary p-1"
                                        aria-label={t('common.close')}><X size={18}/></button>
                            </div>

                            <div className="grid grid-cols-[120px_1fr] gap-4">
                                {resolveCardImageUrl(selectedCard) ? (
                                    <img src={resolveCardImageUrl(selectedCard)} alt={selectedCard.name}
                                         className="w-full rounded-xl"/>
                                ) : (
                                    <div
                                        className="aspect-[2.5/3.5] rounded-xl bg-bg-card flex items-center justify-center text-xs text-text-muted text-center p-2">{selectedCard.name}</div>
                                )}
                                <div className="space-y-3 text-sm">
                                    <div className="grid grid-cols-2 gap-2">
                                        <div className="rounded-lg bg-bg-card p-2"><p
                                            className="text-xs text-text-muted">{t('binderTypes.owned')}</p><p
                                            className="font-bold text-green">{selectedCard.owned_quantity || 0}</p>
                                        </div>
                                        <div className="rounded-lg bg-bg-card p-2"><p
                                            className="text-xs text-text-muted">{t('binderTypes.missing')}</p><p
                                            className="font-bold text-brand-red">{selectedCard.missing_quantity || 0}</p>
                                        </div>
                                    </div>
                                    {isWishlist ? (
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button className="btn-ghost px-2"
                                                        onClick={() => changeRequiredQuantity(selectedCard, -1)}
                                                        disabled={updateEntryMutation.isPending || (selectedCard.required_quantity || 1) <= 1}>
                                                    <Minus size={14}/></button>
                                                <span
                                                    className="text-lg font-bold text-text-primary min-w-8 text-center">{selectedCard.required_quantity || 1}</span>
                                                <button className="btn-ghost px-2"
                                                        onClick={() => changeRequiredQuantity(selectedCard, 1)}
                                                        disabled={updateEntryMutation.isPending}><Plus size={14}/>
                                                </button>
                                            </div>
                                        </div>
                                    ) : (
                                    ...
                                    <p className="text-xs text-text-muted">
                                        ...
                                        <span
                                            className="text-green font-semibold">{formatPrice(selectedCard.price_market)}</span>
                                    ) : (
                                        <span>{t('binderTypes.noPriceData')}</span>
                                    )}
                                    </p>
                                    {(selectedCard.variant || selectedCard.condition) &&
                                        <p className="text-xs text-text-muted">{[selectedCard.variant, selectedCard.condition].filter(Boolean).join(' · ')}</p>}
                                </div>
                            </div>
                                    ...
                                    {(equivalentPrintsData?.equivalents || []).length > 0 && (
                                        <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                                            {equivalentPrintsData.equivalents.map((print) => {
                                                ...
                                                return (
                                                    <div key={print.collection_item_id || print.id}
                                                         className={`flex items-center gap-3 rounded-lg border p-2 ${print.is_current ? 'border-yellow/40 bg-yellow/5' : 'border-border bg-bg/40'}`}>
                                                        {imageUrl ? (
                                                            <img src={imageUrl} alt={print.name}
                                                                 className="w-10 aspect-[2.5/3.5] object-cover rounded"
                                                                 loading="lazy"/>
                                                        ) : (
                                                            <div
                                                                className="w-10 aspect-[2.5/3.5] rounded bg-bg-elevated flex items-center justify-center text-[9px] text-text-muted text-center px-1">{print.name}</div>
                                                        )}
                                                        <div className="min-w-0 flex-1">
                                                            <p className="text-xs font-semibold text-text-primary truncate">{print.set_name || print.set_id} #{print.number}</p>
                                                            <div
                                                                className="flex items-center gap-2 flex-wrap text-[11px] text-text-muted">
                                                                {print.lang &&
                                                                    <span>{tcgdexLanguageLabel(print.lang)}</span>}
                                                                ...
                                                                {print.owned && <span
                                                                    className="text-green font-semibold">{t('binderTypes.owned')} {print.owned_quantity}x</span>}
                                                                {isCollection && !print.is_current && print.available_quantity === 0 &&
                                                                    <span
                                                                        className="text-yellow font-semibold">{t('binderTypes.alreadyUsed')}</span>}
                                                                {print.is_current && <span
                                                                    className="text-yellow font-semibold">{t('binderTypes.currentPrint')}</span>}
                                                            </div>
                                                        </div>
                                                        <button
                                                            type="button"
                                                            ...
                                                            onClick={() => switchPrintMutation.mutate({
                                                                binderCardId: selectedCard.binder_card_id,
                                                                cardId: print.id,
                                                                collectionItemId: print.collection_item_id
                                                            })}
                                                        >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                <button className="btn-ghost justify-center" onClick={() => {
                                    ...
                                    if (wishlistQuantity) wishlistMutation.mutate({
                                        binderCardId: selectedCard.binder_card_id,
                                        quantity: wishlistQuantity
                                    })
                                }}>
                                    <Heart
                                        size={16}/> {isWishlist ? t('binderTypes.addMissingToWishlist') : t('binderTypes.addToWishlist')}
                                </button>
                                <button className="btn-ghost justify-center text-brand-red" onClick={() => {
                                ...
                                <button className="btn-primary justify-center"
                                        onClick={() => setSelectedCard(null)}>{t('binderTypes.done')}</button>
                            </div>
                        </div>
                    </div>
                </div>
