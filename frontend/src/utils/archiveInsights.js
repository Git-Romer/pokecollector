...
export function deriveArchiveInsights({recentAdditions = [], totalCards = 0, sets = []}) {
    ...
    for (const set of sortedSets) {
        ...
        if (remaining > 0 && percent >= 90) notes.push({
            id: `near-${set.id}`,
    ...
    for (const card of recentAdditions) {
        if (card?.name) notes.push({
            id: `recent-${card.id || card.card_id || card.name}`,
            kind: 'recent',
            title: 'Noted',
            body: `I've had my eye on ${card.name}.`,
            href: `/collection`
        })
    }
    if (totalCards > 0 && totalCards % 100 === 0) notes.push({
        id: `milestone-${totalCards}`,
        kind: 'milestone',
        title: 'Milestone',
        body: `${totalCards} cards. The archive is growing nicely.`,
        href: '/collection'
    })
