      ...
      <div className="space-y-4 p-5">
        ...
        {!isRemoved && !removing && (
          ...
          >
            ...
            <label>
              <span className="mb-1 block text-xs font-semibold text-text-secondary">Acquisition source</span>
              <select className="select w-full" value={acquisitionSource} onChange={event => setAcquisitionSource(event.target.value)}>
            ...
            <label className="sm:col-span-2">
              <span className="mb-1 block text-xs font-semibold text-text-secondary">Storage location</span>
              <select className="select w-full" value={locationId} onChange={event => setLocationId(event.target.value)} required>
            ...
            <label className="sm:col-span-2">
              ...
              <textarea className="input min-h-24 w-full" value={notes} onChange={event => setNotes(event.target.value)} />
            </label>
            {sealedCondition === 'opened' && (
              <label className="sm:col-span-2 flex cursor-pointer items-start gap-3 rounded-xl border border-yellow/25 bg-yellow/10 p-3">
                <input
                  type="checkbox"
            ...
            <div className="flex flex-col-reverse gap-2 border-t border-border pt-4 sm:col-span-2 sm:flex-row sm:justify-between">
              <button type="button" className="btn-ghost justify-center text-brand-red" onClick={() => setRemoving(true)}>
                <Trash2 size={15} /> Remove
              </button>
              <div className="flex gap-2">
                <button type="button" className="btn-ghost flex-1 justify-center sm:flex-none" onClick={onClose}>Cancel</button>
                <button className="btn-primary flex-1 justify-center sm:flex-none" disabled={updateMutation.isPending}>Save changes</button>
              </div>
            </div>
          </form>
        )}
        ...
        {removing && (
          ...
          >
            <div className="rounded-xl border border-brand-red/25 bg-brand-red/10 p-3">
              ...
              <p className="mt-1 text-xs text-text-secondary">The record and its local change history are preserved.</p>
            </div>
            <label className="block">
              <span className="mb-1 block text-xs font-semibold text-text-secondary">Reason</span>
              <select className="select w-full" value={removalReason} onChange={event => setRemovalReason(event.target.value)}>
            ...
            <label className="block">
              ...
              <textarea className="input min-h-20 w-full" value={removalNotes} onChange={event => setRemovalNotes(event.target.value)} />
            </label>
            <div className="flex justify-end gap-2">
              <button type="button" className="btn-ghost" onClick={() => setRemoving(false)}>Back</button>
              <button className="btn-primary bg-brand-red" disabled={removeMutation.isPending}>Move to history</button>
            </div>
          </form>
        )}
