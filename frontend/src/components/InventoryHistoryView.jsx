            ...
            <div className="min-w-0 flex-1">
              ...
              {(summaries.length > 0 || event.notes) && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {summaries.map(summary => <span key={summary} className="inventory-history-change">{summary}</span>)}
                  {event.notes && <span className="inventory-history-note">{event.notes}</span>}
                </div>
              )}
            </div>
            <span className="hidden font-mono text-[10px] text-text-muted sm:block">{event.entity_uid.slice(0, 8)}</span>
          </article>
        )
      })}
    </div>
