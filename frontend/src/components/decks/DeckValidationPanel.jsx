...
function translatedCheckMessage(check, t, label) {
  const values = check.details || {}
  const status = check.status === 'pass' ? 'Pass' : check.status === 'unavailable' ? 'Unavailable' : 'Fail'
  const keys = {
    deck_size: `decks.validationDeckSize${status}`,
    basic_pokemon: `decks.validationBasicPokemon${status}`,
    copy_limit: `decks.validationCopyLimit${status}`,
    ownership: `decks.validationOwnership${status}`,
    format_legality: `decks.validationFormatLegality${status}`,
  }
  const key = keys[check.code]
  if (!key) return check.message
  const translated = t(key)
  if (translated === key) return check.message
  return label(key, {
    current: values.current ?? 0,
    target: values.target ?? 0,
    count: values.count ?? values.missing ?? values.illegal_cards?.length ?? 0,
    format: values.format ? t(`decks.format${values.format}`) : '',
  })
}
