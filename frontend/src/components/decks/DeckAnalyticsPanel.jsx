...
function Probability({ deckId, entries, t, probabilityFetcher = getDeckProbability, probabilityQueryKey = 'deck-probability' }) {
  ...
  const { data, isLoading } = useQuery({ queryKey: [probabilityQueryKey, String(deckId), hand, draws, prizeCount, cardName], queryFn: () => probabilityFetcher(deckId, { hand, draws, prize_count: prizeCount, card_name: cardName || undefined }).then(response => response.data ?? response) })
