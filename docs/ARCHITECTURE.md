...
4. TCGdex candidates are searched in the detected language with English fallback (a generic Energy-card name is substituted with its elemental type before searching, since the printed text does not match TCGdex's catalogue naming) and ranked deterministically by local number, language, printed total, set code, regulation mark, artist, and HP. A candidate matching the recognized number or set code is protected from the search-result truncation that would otherwise drop it. Missing fields are neutral; contradictions reduce rank.
