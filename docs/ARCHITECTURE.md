...
4. Candidates are searched against the locally synced `cards` table in the detected language with English fallback. A search that finds no local rows for a given (language, name) pair falls back to a live TCGdex call for that pair only — the local sync can lag behind TCGdex (a newly released set, or a full sync still in progress), and that looks identical to "this card does not exist" from the DB's point of view alone. The fallback result is used for that scan only and is not written back to the catalogue. Candidates from either source are then ranked deterministically by local number, language, printed total, set code, regulation mark, artist, and HP. Missing fields are neutral; contradictions reduce rank.
