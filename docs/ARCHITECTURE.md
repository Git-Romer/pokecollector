...
4. Candidates are searched against the locally synced `cards` table (not a live TCGdex call) in the detected language with English fallback, then ranked deterministically by local number, language, printed total, set code, regulation mark, artist, and HP. Missing fields are neutral; contradictions reduce rank. Matching is limited to languages actually synced locally (`TCGDEX_SYNC_LANGUAGES`).
