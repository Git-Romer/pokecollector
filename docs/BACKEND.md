...
4. Candidates are found by querying the locally synced `cards` table (not a live TCGdex call) and ranked deterministically by local number, language, printed total, set code, regulation mark, artist, and HP. Missing evidence is neutral and contradictions are negative. Matching is limited to languages actually synced locally (`TCGDEX_SYNC_LANGUAGES`).
